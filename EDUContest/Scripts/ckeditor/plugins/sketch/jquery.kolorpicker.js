(function($) {
  ...
  var paletteHTML = ''; //cached array of fully printed html for the palettes
  ...
  function generateHTML(id) {
    ...
    for (var palette = 0;  palette < 3; palette++) {
      ...
      for (var row = 0; row < 8; row++) {
        ...
        for (var col =0; col < 12; col++) {
          html[palette] += "<td class='tile' style='font-size:12px;padding:0;margin:0;border:1px solid #333333;cursor:pointer;background:#"+palettes[palette][row][col]+"' id='"+palettes[palette][row][col]+"' data-color='#"+palettes[palette][row][col]+"' href='#"+id+"'>&nbsp</td>";
        }
        html[palette] += '</tr>';
      }
  ...
  window.kolorpicker = function kolorpicker(id){
  
	  paletteHTML = generateHTML(id); //cached array of fully printed html for the palettes
